    ...
    BEGIN
        ...
        IF (tableExists = FALSE) THEN
            CREATE TABLE IF NOT EXISTS ${schema}.LDS_TELEGRAM_USER (
                ...
                CONSTRAINT LDS_UC_TELEGRAM_USER_USERNAME UNIQUE(USER_NAME) USING INDEX TABLESPACE LDS_TS_IDX
            ) TABLESPACE LDS_TS_DATA;

            COMMENT ON TABLE ${schema}.LDS_TELEGRAM_USER                   IS 'Данные о пользователе Telegram';
            COMMENT ON COLUMN ${schema}.LDS_TELEGRAM_USER.TELEGRAM_USER_ID IS 'Уникальный идентификатор записи';
